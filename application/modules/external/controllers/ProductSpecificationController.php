...
class ProductSpecificationController extends BaseController
{
    ...
    private function mapSpecifications($product_id, $specification_data, $language_code)
    {
        ...
        $lifeTimeData = [];
        $isLifetimeHoursMatched = preg_match("/(?<lifetime>\d{1,3},\d{3})/", $specification_data['data']['lifetime'], $lifeTimeData);
        if ((bool)$isLifetimeHoursMatched) {
            $data['lifetime_hours'] = isset($lifeTimeData['lifetime']) ?
                str_replace(',', '', $lifeTimeData['lifetime']) : 0;
        } else {
