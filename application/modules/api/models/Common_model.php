...
//        die;
    ...
    public function getUserList($params) {
        ...
        $this->db->from('ai_user as u');
        if ($userlist_type == 1) {
            $this->db->join('ai_friend_request as f', '((f.sender_id=u.user_id AND f.receiver_id=' . $params['user_id'] . ') OR (f.sender_id=' . $params['user_id'] . ' AND f.receiver_id=u.user_id))', 'left', false);
        } else if ($userlist_type == 2) {
...
//        echo $this->db->last_query();die;
    ...
    public function getUserInfoWithAddress($accessToken, $field = ['u.user_id']) {
        ...
        $userInfo = $this->getCurrentUserInfoWithAddress($field, $whereArr);
        if (!empty($userInfo)) {
    ...
    public function EemployeeRequestsbyUser($fields = 'erm.*,u.*', $userid, $companyid = false, $reueststatus = 0) {
        ...
        $this->db->where('erm.status', $reueststatus);
        if ($companyid) {
