...
class Profile extends REST_Controller {
    ...
    public function companylist_get() {
        ...
        */                
        try {                     
            $whereArr['where'] = ['owner_type !='=>'1','status' =>'1'];
            if($getDataArr['crn']){
                $whereArr['like'] = ['company_reg_number'=>$getDataArr['crn']];
            }
            $companyList =  $this->Common_model->fetch_data('company_master', 'company_id,company_name,company_reg_number,company_image,company_image_thumb', $whereArr, false);
            //echo $this->db->last_query(); die;
            if (!empty($companyList)) {                
                $this->response(array('code' => SUCCESS_CODE, 'msg' => $this->lang->line('process_success'),'result' => $companyList));
            } else {                
     ...
     public function profileupdate_post() {
        ...
        if ($this->form_validation->run()) {
            ...
            try {
                $this->load->library('commonfn');
                $respArr = $this->Common_model->getUserInfo($postDataArr['accesstoken'], ['u.user_id','u.country_id as u_country_id','sl.id as u_state_id','sl.state_code as u_state_code','u.city_id as u_city_id','u.zipcode','cl.name as u_country_name','sl.name as u_state_name','cyl.name as u_city_name','login_time','login_status','status','first_name','middle_name','last_name','email','prm_user_countrycode','phone','alt_user_countrycode','alt_userphone','company_id','is_owner','user_type','IF(image !="",CONCAT("' . IMAGE_PATH . '","",image),"") as image','IF(image_thumb !="",CONCAT("' . THUMB_IMAGE_PATH . '","",image_thumb),"") as image_thumb']);
                //echo $this->db->last_query(); die;
                $user_info = [];
                ...
                if ($this->db->trans_status() === TRUE) {
                    ...
                    $userdata = $this->Common_model->getUserInfo($postDataArr['accesstoken'], ['u.user_id','u.country_id as u_country_id','sl.id as u_state_id','sl.state_code as u_state_code','u.city_id as u_city_id','u.zipcode','cl.name as u_country_name','sl.name as u_state_name','sl.state_code as u_state_code','sl.id as u_state_id','cyl.name as u_city_name','login_time','login_status','status','first_name','middle_name','last_name','email','prm_user_countrycode','phone','alt_user_countrycode','alt_userphone','company_id','is_owner','user_type','IF(image !="",image,"") as image','IF(image_thumb !="",image_thumb,"") as image_thumb']);
                    unset($userdata['code']);
                    //$whereArr['where'] = ['company_id'=>$user_info['company_id']];
                    //$companyDetail =  $this->Common_model->fetch_data('company_master', 'company_id,company_name,company_reg_number,company_image,company_image_thumb', $whereArr, true);
